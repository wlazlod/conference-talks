...
\setbeamertemplate{footline}{%
  \hbox to \paperwidth{\hspace*{0.6cm}\hfill
...
\begin{tikzpicture}[font=\scriptsize]
  ...
  \node[text=ink,anchor=south] at (1.8,3.12){\scriptsize +delay};
  % +overdue (up, coral)
...
\begin{tikzpicture}[font=\scriptsize]
  ...
  \foreach \i/\h in {0/1.2,1/0.7,2/1.5,3/0.5,4/0.9}{
     \fill[muted] ({\i*0.34},0) rectangle ({\i*0.34+0.24},\h);}
  \node[text=muted,anchor=south] at (0.7,1.7){Mon};
...
\begin{tikzpicture}[font=\scriptsize]
  ...
  \foreach \i/\name/\w/\c in {
     0/{age}/3.2/gold,
     1/{max delay 3m}/1.4/muted,
     2/{max delay 6m}/1.3/muted,
     3/{max delay 12m}/1.1/muted,
     4/{overdue max}/1.0/muted,
     5/{overdue avg}/0.8/muted}{
...
\begin{tikzpicture}[font=\tiny]
  ...
  \fill[muted] (0,1.35) rectangle (1.3,1.57);
  \node[text=muted,anchor=west] at (1.35,1.46){age};
  \foreach \i/\w in {0/0.6,1/0.5,2/0.45,3/0.35}{
     \pgfmathsetmacro\yy{1.0-\i*0.30}
     \fill[gold] (0,\yy) rectangle (\w,\yy+0.22);}
  \node[text=gold,anchor=west] at (0.7,-0.18){delay family, split};
  % arrow
  \draw[muted,->,>=stealth,line width=1pt] (2.55,0.9) -- (3.25,0.9);
  \node[text=muted,anchor=south,font=\tiny] at (2.9,0.95){group};
  % ----- grouped -----
  \begin{scope}[xshift=3.6cm]
   \node[text=muted,anchor=south] at (0.9,2.05){grouped};
   \node[text=gold,anchor=south,font=\tiny] at (0.85,1.62){delinquency};
   \fill[gold] (0,1.35) rectangle (1.7,1.57);
   \fill[muted] (0,1.0) rectangle (0.9,1.22);
   \node[text=muted,anchor=west] at (0.95,1.11){age};
  \end{scope}
\end{tikzpicture}}
...
\newcommand{\unrealviz}{%
\begin{tikzpicture}[font=\tiny]
  \draw[muted,->,>=stealth] (0,0)--(3.0,0) node[anchor=north,text=muted,font=\tiny]{delay 3m};
  \draw[muted,->,>=stealth] (0,0)--(0,2.6) node[anchor=south,text=muted,font=\tiny]{delay 6m};
...
% Face 2 remedy: a rare feature is low on mean |SHAP| but high on max |SHAP|
\newcommand{\meanmaximp}{%
\begin{tikzpicture}[font=\tiny]
  % mean |SHAP|
  \node[text=muted,anchor=south] at (0.9,1.95){mean $|$SHAP$|$};
  \fill[muted] (0,1.55) rectangle (1.7,1.75);
  \fill[muted] (0,1.25) rectangle (1.3,1.45);
  \fill[muted] (0,0.95) rectangle (1.0,1.15);
  \fill[gold]  (0,0.65) rectangle (0.3,0.85);
  \node[text=gold,anchor=west] at (0.35,0.75){phone-reuse};
  % arrow
  \draw[mint,->,>=stealth,line width=1pt] (3.05,1.2)--(3.95,1.2);
  \node[text=mint,anchor=south,font=\tiny] at (3.5,1.25){look at max};
  % max |SHAP|
  \begin{scope}[xshift=4.35cm]
   \node[text=muted,anchor=south] at (0.9,1.95){max $|$SHAP$|$};
   \fill[gold]  (0,1.55) rectangle (1.8,1.75);
   \node[text=gold,anchor=west] at (1.85,1.65){phone-reuse};
   \fill[muted] (0,1.25) rectangle (0.9,1.45);
   \fill[muted] (0,0.95) rectangle (0.7,1.15);
   \fill[muted] (0,0.65) rectangle (0.5,0.85);
  \end{scope}
\end{tikzpicture}}
...
record, max delay (worst payment delay in a look-back window), overdue amounts,
utilization, age.\\[14pt]
\begin{columns}[c]
\column{0.46\textwidth}
  ...
  \normalsize Higher predictive power, catches hidden risk --- \textcolor{coral}{but it's a
  black box.}
\column{0.5\textwidth}
...
% ---------- 4. WHY EXPLANATIONS ------------------------------------
\begin{frame}{Why do we even need to explain it}
\large
...
\begin{rulebox}
SHAP tells you \textbf{why} the model gave this score. It does \emph{not} tell you
\textbf{what to change} to get a different one --- for that, use counterfactuals.
\end{rulebox}
...
\column{0.5\textwidth}
  ...
  SHAP is always measured against a \emph{reference set} of customers --- the base
  value on that waterfall \emph{is} this background. Swap it or let it drift and
  every SHAP moves, even with identical inputs.
\end{columns}
\vspace{12pt}
\begin{warnbox}
...
\begin{columns}[c]
\column{0.5\textwidth}
A \textbf{family} of features for one phenomenon: max delay at 3/6/9/12 months,
...
exist} --- max delay $=90$ days in one window, $0$ in the next --- and scores the model on
...
\begin{frame}{Face 1 --- why: SHAP builds rows that can't exist}
\large
Two correlated features --- \textbf{max delay over 3m} and \textbf{over 6m} ---
where by construction $6\text{m}\ge 3\text{m}$. To score ``delay 3m'' alone, SHAP
keeps it and draws ``delay 6m'' from the background:
\vspace{8pt}
\begin{columns}[T]
\column{0.5\textwidth}
\begin{rulebox}
\textbf{Real customer}\\[4pt]
delay 3m $=90$,\ \ delay 6m $=95$\\[4pt]
{\normalsize\textcolor{mint}{\ding{51}}\ plausible}
\end{rulebox}
\column{0.5\textwidth}
\begin{warnbox}
\textbf{Row SHAP scores}\\[4pt]
delay 3m $=90$,\ \ delay 6m $=0$\\[4pt]
{\normalsize\textcolor{coral}{\ding{55}}\ impossible ($6\text{m}<3\text{m}$)}
\end{warnbox}
\end{columns}
\vspace{12pt}
\centering\textcolor{ink}{The model is queried on that impossible row --- and what
it predicts there leaks into the explanation.}
\end{frame}
...
the \emph{delinquency phenomenon} against age, not one-of-five delay features against age.
...
\large
Take a normally-quiet signal --- e.g.\ \textbf{\# applications sharing one phone
number across different PESELs}. In training it's almost always zero, so by mean
$|$SHAP$|$ it ranks among the \textbf{least important} features.\\[2pt]
\begin{columns}[c]
\column{0.56\textwidth}
  \centering
  \setlength{\tabcolsep}{0.3cm}%
  \begin{tabular}{ccc}
    {\fontsize{34}{38}\selectfont\bfseries\textcolor{gold}{99.5\%}} &
    \raisebox{7pt}{\fontsize{30}{30}\selectfont\textcolor{muted}{$\Longrightarrow$}} &
    {\fontsize{34}{38}\selectfont\bfseries\textcolor{gold}{90\%}} \\[3pt]
    {\small\textcolor{muted}{zeros in training}} & &
    {\small\textcolor{muted}{zeros in production}}
  \end{tabular}
...
\vspace{2pt}
\begin{warnbox}
Then operators start \textbf{recycling} phone numbers --- the same number
legitimately reappears across people. The feature quietly goes from 99.5\% to 90\%
zeros and turns into a real driver --- but it sat at the bottom of the ranking, so
nobody was watching.
\end{warnbox}
\end{frame}

\begin{frame}{Face 2 --- the remedy}
\large
A rare feature can be \emph{weak on average but decisive when it fires}: low mean
$|$SHAP$|$, high \textbf{maximum} $|$SHAP$|$.\\[4pt]
\begin{center}\meanmaximp\end{center}
\begin{rulebox}
Rank and monitor features by the \textbf{max} (or the tail) of $|$SHAP$|$, not only
the mean --- that's where rare-but-powerful drivers hide.
\end{rulebox}
\end{frame}
...
\vspace{14pt}
\begin{rulebox}
Two definitions, not a bug --- so decide what you need importance \emph{for}, then
pick the method.
\end{rulebox}
...
\end{center}
\begin{rulebox}
\textbf{Competitive}, not ``better'' --- trade one AUROC point for full transparency.
\end{rulebox}
...
\begin{tikzpicture}[remember picture,overlay]
  \fill[nightbg] (current page.south west) rectangle (current page.north east);
  % bar motif tucked into the bottom-right corner, clear of the text
  \foreach \x/\h/\c in {13.0/1.8/coral, 13.6/2.9/coral, 14.2/1.2/mint, 14.8/3.3/mint, 15.4/2.0/gold}{
     \fill[\c, opacity=0.18] ($(current page.south west)+(\x,0.55)$) rectangle ++(0.45,\h);}
  \node[anchor=north west, text=ink, align=left] at ($(current page.west)+(1.0,3.05)$)
       {\fontsize{36}{44}\selectfont\bfseries\begin{minipage}{13cm}\raggedright
        Love SHAP.\\ Ask only what it answers.\end{minipage}};
  \node[anchor=west, text=gold, align=left] at ($(current page.west)+(1.1,-2.45)$)
       {\Large\begin{minipage}{11cm}\raggedright When the stakes are high --- consider a glass box.\end{minipage}};
  \node[anchor=west, text=muted] at ($(current page.west)+(1.12,-3.6)$)
...
\begin{frame}[c]{}
\centering
{\Large\textcolor{ink}{\bfseries Slides \& references}}\\[14pt]
\colorbox{white}{\hspace{4pt}\raisebox{-4pt}{\includegraphics[height=4.1cm]{qr_slides}}\hspace{4pt}}\\[12pt]
{\small\textcolor{gold}{github.com/wlazlod/conference-talks}}\\[3pt]
{\footnotesize\textcolor{muted}{The Scorecard Illusion --- DSS AI Edition 2026}}
\end{frame}
